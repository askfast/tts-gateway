...
Voicerss.fetchAudio = function(writestream, options, keyIndex, response, users) {
    ...
    logger.info('urlWithKey: ' + urlWithKey);
    ...
    var x = request.get({
        ...
        if (res.headers['content-type'].indexOf('audio/wav') == -1) {
            res.on('data', function(chunk) {
                logger.error('Audio not fetched with Key: ' + key + ' and Index: ' + keyIndex + '. Response is: ' + chunk);
            });
            if (keyIndex < KEYS.length - 1) {
                ...
                logger.error('No audio fetched. Returning default audio');
