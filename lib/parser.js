 ...
 */
 ...
 var Parser = {};
var KEYS = ['', '']; // ADD YOUR VOICE RSS API KEYS HERE!!
...
function fetchAudio(writestream, urlWithoutKey, keyIndex) {
    ...
    var x = request.get(urlWithKey).on('response', function parseFetchAudioResponse(res) {
            ...
            if(res.headers['content-type'].indexOf('audio/wav')==-1) {
                console.log('Audio not fetched with Key: ' + KEYS[keyIndex] + ' and Index: '+ keyIndex + '. Response is: ' + res );
                    if(keyIndex < KEYS.length) {
                        fetchAudio(writestream, urlWithoutKey, ++keyIndex);
                    }
                    else{
                        x.pipe(res);
                    }
