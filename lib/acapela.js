...
Acapela.fetchAudio = function(writestream, options, response) {
    ...
    request.post(reqOptions,function(error,response,body){
        ...
        var parsedRes = JSON.parse('{"' + decodeURI(body.replace(/&/g, "\",\"").replace(/=/g,"\":\"")) + '"}');
            ...
            var x = request.get({headers: { "User-Agent": "AskFastAudioGateway/1.0"}, url: parsedRes.snd_url}).on('response', function parseFetchAudioResponse(res) {
