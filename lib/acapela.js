...
//TODO set keys
var username = config.acapela.username
var password = config.acapela.password
var login = config.acapela.login
...
Acapela.fetchAudio = function(writestream, options, resp, user) {
    ...
    if (user) {
        login = user.properties && user.properties.login ? user.properties.login : null;
